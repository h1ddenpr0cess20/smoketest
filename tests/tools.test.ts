...
describe("provider tool building", () => {
  ...
  it("builds the OpenAI set (no x_search, file_search needs store ids)", () => {
    const types = buildTools("openai", everything).map((tool) => tool.type);
    expect(types).toEqual(["web_search", "code_interpreter", "file_search", "mcp"]);
    const withoutStores = buildTools("openai", { ...everything, vectorStoreIds: [] }).map((tool) => tool.type);
  ...
  it("emits MCP servers with approval disabled and rejects bad entries", () => {
    const [mcp] = buildTools("openai", { mcpServers: [{ label: "workspace", url: "http://localhost:9404/mcp" }] });
    expect(mcp).toEqual({
      type: "mcp",
    ...
    expect(buildTools("openai", { mcpServers: [{ label: "bad label!", url: "http://x/mcp" }] })).toEqual([]);
    expect(buildTools("openai", { mcpServers: [{ label: "ok", url: "ftp://x/mcp" }] })).toEqual([]);
  });
});
  ...
  it("labels search calls with their queries", () => {
    expect(
      toolActivity({
        ...
        item: { id: "ws_1", type: "web_search_call", action: { queries: ["next.js 16", "webpack"] } },
      }),
  ...
  it("labels MCP calls with the tool name", () => {
    expect(
      toolActivity({
        ...
        item: { id: "mcp_1", type: "mcp_call", name: "read_file", server_label: "workspace" },
      }),
    ).toEqual({ id: "mcp_1", label: "MCP · read_file" });
  });

  it("ignores plain message items and other events", () => {
    expect(toolActivity({ type: "response.output_item.added", item: { id: "m1", type: "message" } })).toBeNull();
    expect(toolActivity({ type: "response.output_text.delta", delta: "x" })).toBeNull();
  });
});
