  ...
  while (start < t.length) {
    ...
    if (breakAt < 0) {
      ...
      if (sentMatches.length) {
        ...
        if ((last.index ?? -1) >= safeSize * 0.4)
          breakAt = (last.index ?? 0) + last[0].length;
      }
    }
...
export function isDocumentInventoryQuery(query: string): boolean {
  ...
  return (
    ...
    /\b(?:files?|documents?|sources?)\b.{0,32}\b(?:available|attached|indexed|uploaded|access)\b/.test(
      normalized,
    )
  );
}
...
function lexicalScores(chunks: RagChunk[], query: string): number[] {
  ...
  const documents = chunks.map((chunk) =>
    lexicalTokens(`source ${chunk.name}\n${chunk.text}`),
  );
  const avgLength =
  ...
  const raw = documents.map((terms) => {
    ...
    for (const term of queryTerms) {
      ...
      const denominator =
        tf + BM25_K1 * (1 - BM25_B + (BM25_B * terms.length) / avgLength);
...
function diversifyCandidates(
  ...
  const sourceTotal = new Set(
    candidates.map((candidate) => candidate.chunk.name),
  ).size;
  const perSourceLimit = inventoryQuery
    ? 1
    : Math.max(
        MIN_MULTI_SOURCE_CHUNK_LIMIT,
        Math.ceil(topK / Math.max(1, Math.min(sourceTotal, 4))),
      );
  let characters = 0;
  ...
  while (selected.length < topK) {
    ...
    for (const candidate of candidates) {
      ...
      if (
        selected.length > 0 &&
        characters + candidate.chunk.text.length > characterBudget
      )
        continue;
      ...
      const redundancy =
        selected.length === 0
          ...
          : Math.max(
              ...selected.map((item) =>
                normalizedCosine(candidate.chunk.vector, item.chunk.vector),
              ),
            );
      const sourcePenalty = sourceCount * 0.12;
      const mmrScore =
        MMR_RELEVANCE_WEIGHT * candidate.relevance -
    ...
    sourceCounts.set(
      best.chunk.name,
      (sourceCounts.get(best.chunk.name) || 0) + 1,
    );
...
export function rankChunks(
  ...
  const candidates = chunks
    .map((chunk, i): RetrievalCandidate => {
      ...
      return {
        ...
        lexical,
        relevance:
          HYBRID_DENSE_WEIGHT * dense + HYBRID_LEXICAL_WEIGHT * lexical,
      };
    })
  ...
  const minimumRelevance = inventoryQuery
    ? 0
    : Math.max(0.05, bestRelevance * 0.35);
  const poolSize = Math.max(topK * 5, 40);
  const candidatePool = candidates
    .filter(
      (candidate) =>
        candidate.relevance >= minimumRelevance || candidate.lexical > 0,
    )
    .slice(0, poolSize);
